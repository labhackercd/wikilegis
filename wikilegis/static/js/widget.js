...
$('head')
  .append('<link rel="stylesheet" type="text/css" href="' + domain + 'static/css/widget.css">');
$(document)
  .ready(function() {
    loadBill(bill_id);
  });
$('.wikilegis-widget')
  .wrap("<div class='wikilegis-widget-wrapper'></div>")
  ...
  $.each($('.create-proposal'), function(index, form) {
    ...
    $(form)
      ...
      .append($(document.createElement('textarea'))
        ...
        .attr('cols', '40')
        .attr('rows', '10')
        .val($('.original.segment-' + segment_id)
          .attr('data-raw-content')))
      .append($(document.createElement('button'))
...
function hideForms() {
  $.each($('.create-comment'), function(index, form) {
    $(form)
      .html('')
      .append($(document.createElement('p'))
        .html('Fa&ccedil;a o login para comentar.'));
  });
  $.each($('.create-proposal'), function(index, form) {
    $(form)
      .html('')
      .append($(document.createElement('p'))
        .html('Fa&ccedil;a o login para sugerir uma proposta.'));
  });
};
...
function loginWikilegis() {
  var username = $("#login-wikilegis input[name=username]")
    .val();
  var password = $("#login-wikilegis input[name=password]")
    .val();
  $.post(domain + 'accounts/api-token-auth/', {
      username: username,
      password: password
    }, function(data) {
    ...
    .error(function() {
      $('#login-wikilegis')
        .append($(document.createElement('p'))
          .html('Usu&aacute;rio ou senha inv&aacute;lidos!'));
    })
};

function registrationWikilegis() {
  var first_name = $("#registration-wikilegis input[name=first_name]")
    .val();
  var last_name = $("#registration-wikilegis input[name=last_name]")
    .val();
  var email = $("#registration-wikilegis input[name=email]")
    .val();
  var password = $("#registration-wikilegis input[name=password]")
    .val();
  $.post(domain + 'api/user/create/', {
    ...
    .done(function(data) {
      $('#registration-wikilegis')
        .append($(document.createElement('p'))
          .html('Conta criada com sucesso! Fa&ccedil;a o login.'));
    })
    .error(function() {
      $('#registration-wikilegis')
        .append($(document.createElement('p'))
          .html('Dados inv&aacute;lidos!'));
    })
};
...
function addDiff() {
  $('.original')
    .each(function() {
      ...
      var original = $(div_original)
        .attr('data-raw-content');
      $(div_original)
        .children('.proposals')
        .children('.segment-proposal')
        .children('.content')
        .each(function(i, cur) {
          var $cur = $(cur);
          var current = $cur.attr('data-raw-content');
          $cur.find('.pp')
            .html(changesToMarkup(wlDiff(original, current)));
        });
      $('.pp')
        .next('p')
        .remove();
...
function get_votes(votes, segment_id) {
  ...
  return $(document.createElement('div'))
    .addClass('votes')
    .html('<i class="material-icons" onclick=vote("up",' + segment_id + ');>thumb_up</i>' + up_votes + ' <i class="tiny material-icons" onclick=vote("down",' + segment_id + ');>thumb_down</i>' + down_votes)
}
...
function vote(bool, segment_id) {
  ...
  $.post(domain + 'api/votes/', {
      vote: bool,
      object_id: segment_id,
      token: $.cookie('wikilegis-token')
    })
    .done(function(data) {
      var up_votes = 0,
        down_votes = 0;
  ...
  $.post(domain + 'api/comments/', {
    ...
    .done(function(data) {
      $(document.createElement('div'))
        ...
        .append(data.comment)
        .insertBefore($('.comments[segment-id=' + segment_id + ']')
...
function proposal(bill_id, segment_id) {
  $.post(domain + 'api/segments/', {
      content: $('#proposal-' + segment_id)
        .val(),
      replaced: segment_id,
      bill: bill_id,
      token: $.cookie('wikilegis-token')
    ...
    .done(function(data) {
      $(document.createElement('div'))
        ...
        .insertBefore($('.proposals[segment-id=' + segment_id + ']')
      ...
      addDiff();
      createForms();
      $('.comments[segment-id=' + data.id + ']')
        .hide();
      $('.segment-proposal.segment-' + data.id)
        .children(".commentCountWrapper")
        .children(".commentCount")
        .on('click', function() {
          $(this)
...
function loadBill(bill_id) {
  $.getJSON(domain + 'api/bills/' + bill_id + '?format=json', function(data) {
      ...
      $('.wikilegis-widget .segment-content')
        .wrap("<a href='" + domain + "bill/" + data.id + "' class='placeholder-link' title='Ver no Wikilegis'></a>");
      $('.wikilegis-widget .title')
        .wrap("<a href='" + domain + "bill/" + data.id + "' class='placeholder-link' title='Ver no Wikilegis'></a>");
      $('.wikilegis-widget .link')
        .remove();
    })
    .done(function() {
      if (getCookie('wikilegis-token') == '') {
      ...
      if ($('.wikilegis-widget .segment')
        .length > 19) {
        var firstSegmentsHeight = 0;
        $('.segment.original:lt(21)')
          .each(function() {
            firstSegmentsHeight += $(this)
              .outerHeight(true);
          });
        $('.wikilegis-widget')
